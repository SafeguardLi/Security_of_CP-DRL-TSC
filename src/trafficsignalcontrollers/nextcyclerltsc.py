'''
NextCycleRLTSC: be able to give next_phase and next_phase_duration and to store expeirence
This also requires a corresponding neural network whose output can be our phase split (perhaps, softmax could work/ or we just normalize the result)
'''
import random
import numpy as np
...
class NextCycleRLTSC(TrafficSignalController):
    def __init__(self, conn, tsc_id, mode, netdata, red_t, yellow_t, green_t, rlagent, detect_r):
    ...
    def get_next_phase(self):
        ...
        if self.empty_intersection():
            ...
            # and one hot vector encoding the previous phase
            # state = np.concatenate( [self.get_state(), self.phase_to_one_hot[self.phase]] )[np.newaxis,...]
            state = np.concatenate([self.get_state(), self.phase_to_one_hot[self.phase]])
            if self.acting:
                terminal = False
