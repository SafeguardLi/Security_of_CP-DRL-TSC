...
class WebstersTSC(TrafficSignalController):
    def __init__(self, conn, tsc_id, mode, netdata, red_t, yellow_t, g_min, c_min, c_max, sat_flow, update_freq, detect_r):
    ...
    def update(self, global_data, cv_data, uv_data, mask):
        if mask:
            data = cv_data
        else:
            data = global_data
