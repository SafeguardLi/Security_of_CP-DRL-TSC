...
class TrafficSignalController:
    ...
    def __init__(self, conn, tsc_id, mode, netdata, red_t, yellow_t, detect_r):
        ...
        print("######self.phase_lanes",self.phase_lanes) ####
        #create subscription for this traffic signal junction to gather
        #vehicle information efficiently
        self.detect_radius = detect_r # it is the communication range between TSC and vehicles
        ...
        self.get_SR_lanes(self.id) # 112024, add left turning lane
    ...
    def get_conn_lanes(self, lane_id, direction):
        # TODO: 2024/11/19 need to test functionality under new phase setting
    ...
    def get_L_lanes(self,tlid, map_name="plymouth"):
        ...
        if map_name in ["plymoth",'plymouth']:
            if tlid == "79":
                self.L_lanes = ['3.0.00_3','-11.0.00_3','-17.0.00_2','16.0.00_2','16.0.00_2']
        elif map_name == "simp_plymouth":
            if tlid == "J1":
                self.L_lanes = ['-E6_2','-E4_2','E5_2','E3_2']
        else:
    ...
    def run(self, con_veh_ls, unequipped_veh_ls, cav_ls, mask, time, estimate_queue, act_ctm, act_lp, attacker = None):
        ...
        ####### QUEUE ESTIMATION #######
        # if (time%10 == 0) and estimate_queue:
        #     # update historic_data every 10 s
        #     self.update_historic_data(cv_data, self.t)

        # wz: record the real number of vehicles on lane to evaluate the QE
        ...
        if act_lp:
            # if we want to disable lp, we can input None here
        ...
        if act_ctm and (self.t%10 == 0):
            ...
            # if (self.mode == 'test') and (self.t%100 == 0):
            #     # if testing, create CTM figure to show real-time result
            #     self.CTM.viz_CTM_matrix(int(self.t//10),"realtime_")
    ...
    def get_tl_green_phases(self):
        ...
        return sorted(green_phases, reverse=True)
    ...
    def get_state(self,tsc_type = None, num_segments=1,act_ctm=False): # add the tsc_type to provide different state setting
        ...
        if tsc_type in ['cavlight','mmitiss']:
            ...
            #                             self.get_num_vehicle_cav(num_segments,'critic','inc'),
            #                             self.get_num_vehicle_cav(num_segments,'critic','out')]),
            #             self.CTM.get_state_CTM(int(self.t//10)))
            # else: 
            return (np.concatenate([self.get_avg_speed(num_segments,'critic'),
                                    # self.get_avg_delay('critic'),
                    ...
                    np.concatenate([self.get_avg_speed(num_segments,'critic')[:10],
                                    # self.get_avg_delay('critic'),
                                    self.get_num_vehicle_cav(num_segments,'critic','inc')[:10]])) #, self.get_num_vehicle_cav(num_segments,'actor','out')

        else:
            state = np.concatenate([self.get_normalized_density(), self.get_normalized_queue()])
    ...
    def get_num_vehicle_cav(self,  num_segments=1, agent=None, lane_dir=None):
        ...
        # TODO: remove the norm part to attacker's state generation process; or save the norm value
        # it would be better to save the norm, so we can better compare the attacked state and normal state
        if (num_segments == 1) or (lane_dir == "out"):
            if agent == "critic":
                ...
                self.norm_CV = np.linalg.norm(np.array(num_cv))
                if self.norm_CV == 0:
                    self.norm_CV = 1
                return np.array(num_cv)/self.norm_CV
            else:
                raise NotImplementedError('get_num_vehicle wrong input:', agent)
        else:
            if num_segments != 3:
                print("WARNINIG! WE HARD CODE NUM_SEGMENTS as 3 NOW, input is",num_segments)
            ...
            if agent == 'critic':
                        ...
                        for lane in self.max_pressure_lanes[phase][lane_dir]:
                            if lane in self.cv_data.keys():
                                for car, info in self.cv_data[lane].items():
                                    ...
                                    dist2sig = self.conn.vehicle.getNextTLS(car)
                                    if len(dist2sig) > 0:
                                        pos = dist2sig[0][2]
                                    if pos <= self.detect_radius:
                                        # --- FIX: Clamp index to avoid Out Of Bounds ---
                ...
                norm_CV = np.linalg.norm(np.hstack(cv_state))
                norm_UV = np.linalg.norm(np.hstack(uv_state))
                if norm_CV == 0:
                    norm_CV = 1
                if norm_UV == 0:
                    norm_UV = 1

                self.state_comparison = [np.hstack(cv_state),np.hstack(uv_state)] # store num of veh info

                # print("Critic num_veh before norm: CV", cv_state, "\n UV:", uv_state) ###

                return np.concatenate([np.hstack(cv_state)/norm_CV,
                                    np.hstack(uv_state)/norm_UV])
                ...
                cv_state = np.array([np.array(a) + np.array(b) for a, b in zip(cv_state, fv_state)], dtype=object)

                self.norm_CV = np.linalg.norm(np.hstack(cv_state))
                if self.norm_CV == 0:
                    self.norm_CV = 1
                return np.hstack(cv_state)/self.norm_CV
            else:
                raise NotImplementedError('get_num_vehicle wrong input:', agent)
    ...
    def get_avg_speed(self, num_segments=1, agent=None):
        ...
        if agent == "critic":
                ...
                if phase in ['rrrGrrrrrrrrGrrrrr']:
                    ...
                    for lane in self.max_pressure_lanes[phase]['inc']:
                        if lane in self.cv_data.keys():
                            for car, info in self.cv_data[lane].items():
                                cv_cnt[i][-1] += 1
                                cv_speed[i][-1] += info[traci.constants.VAR_SPEED]
                        else:
                            empty_cv_lane += 1
                            if empty_cv_lane == num_lane_phase:
                                cv_speed[i][-1] = 0.0 # Initialized to 0.0 to prevent masking real congestion
                            
                        if lane in self.uv_data.keys():
                            for car, info in self.uv_data[lane].items():
                                uv_cnt[i][-1] += 1
                                uv_speed[i][-1] += info[traci.constants.VAR_SPEED]
                        else:
                            empty_uv_lane += 1
            ...
            # Vectorized calculation with mask to handle empty road segments correctly
            cv_cnt_flat = np.hstack(cv_cnt).astype(float) #
            cv_speed_flat = np.hstack(cv_speed).astype(float) #
            uv_cnt_flat = np.hstack(uv_cnt).astype(float) #
            uv_speed_flat = np.hstack(uv_speed).astype(float) #

            avg_spd_CV = np.full_like(cv_cnt_flat, 17.88) # Default to free-flow
            avg_spd_UV = np.full_like(uv_cnt_flat, 17.88) #

            # Only update average for segments where vehicles are present
            has_cv = cv_cnt_flat > 0 #
            has_uv = uv_cnt_flat > 0 #
            avg_spd_CV[has_cv] = cv_speed_flat[has_cv] / cv_cnt_flat[has_cv] #
            avg_spd_UV[has_uv] = uv_speed_flat[has_uv] / uv_cnt_flat[has_uv] #

            # Speed difference from free-flow (17.88)
            diff_CV = np.maximum(0, 17.88 - avg_spd_CV) #
            diff_UV = np.maximum(0, 17.88 - avg_spd_UV) #

            norm_CV = np.linalg.norm(diff_CV) or 1.0 #
            norm_UV = np.linalg.norm(diff_UV) or 1.0 #

            return np.concatenate([diff_CV/norm_CV + 0.2, diff_UV/norm_UV + 0.2]) #
                    ...
                    for lane in self.max_pressure_lanes[phase]['inc']:
                        if lane in self.cv_data.keys():
                            for car, info in self.cv_data[lane].items():
                                cv_cnt[i][-1] += 1
                                cv_speed[i][-1] += info[traci.constants.VAR_SPEED]
                    # Note: fv_speed/cv_speed left at 0 if no vehicles
                else:
                    for lane in self.max_pressure_lanes[phase]['inc']:
                        if lane in self.cv_data.keys():
                            for car, info in self.cv_data[lane].items():
            ...
            v_speed = np.array([np.array(a) + np.array(b) for a, b in zip(fv_speed, cv_speed)], dtype=object) #

            v_cnt_flat = np.hstack(v_cnt).astype(float) #
            v_speed_flat = np.hstack(v_speed).astype(float) #

            # Improved Average Speed Calculation
            perceived_avg_speed = np.full_like(v_cnt_flat, 17.88) # Default to 17.88
            has_vehs = v_cnt_flat > 0 #
            perceived_avg_speed[has_vehs] = v_speed_flat[has_vehs] / v_cnt_flat[has_vehs] #

            # Calculate Speed Drop
            avg_spd_V = np.maximum(0, 17.88 - perceived_avg_speed) #
            
            self.norm_V_spd = np.linalg.norm(avg_spd_V) or 1.0 #
            return avg_spd_V/self.norm_V_spd + 0.2 #
            ...
            if global_critic == 'total':
                ...
                for i,lane in enumerate(self.incoming_lanes):
                    for car,info in self.data[lane].items():
                        ...
                        # --- FIX: Clamp index to avoid Out Of Bounds ---
                        ind = min(num_segments - 1,int(pos//(self.detect_radius/num_segments))) # by default, 150 is the detect_radius
                        # -----------------------------------------------
                        state[i][ind]+=1
    ...
    def get_reward_density(self, tsc_type=None):
        ...
        if tsc_type in ['cavlight']:
            ...
            for g in self.max_pressure_lanes.keys():
                ...
                for l in inc_lanes:
                    if l in self.data:
                        ...
                        for v in self.data[l].keys():
                            ...
                            # pos = max(0,np.linalg.norm(self.junc_position - veh_position)-20)
                            dist2sig = self.conn.vehicle.getNextTLS(v)
                            if len(dist2sig) > 0:
                                pos = dist2sig[0][2]
                ...
                out_pressure = 0 #(sum(out_veh_position)/200.0) #*7.5/sum(self.tot_out_lane_len[g].values())
