...
class VehicleGen:
    def __init__(self, netdata, sim_len, demand, scale, mode, conn):
        np.random.seed()
        self.conn = conn
        # self.v_data = None # wz: comment out v_data
        ...
        if demand == 'single':
            self.gen_vehicles = self.gen_single # wz: only pass the method but not call the function
        elif demand == 'dynamic':
    ...
    def gen_dynamic(self):
        ...
        try:
            new_veh_edges = next(self.v_schedule) # at each time, only generate vehicles in the next step
    ...
    def gen_dynamic_demand(self, mode):
        '''
        generate the number of newly insert vehicles based on a sine function
        and then randomly choose an origin for each of them
        :param mode:
        :return: a list of origins with a length of the number of new vehicles
        '''
    ...
    def set_veh_route(self, veh):
        ...
        route = [current_edge]
        # wz: in a while loop, to generate the whole route by randomly select the next outgoing lanes
